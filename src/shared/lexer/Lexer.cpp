...
static TokenType identifier_type()
{
    ...
    case 'b':
        if (scanner.current - scanner.start > 1)
        {
            ...
            {
            case 'o': return check_keyword(2 , 2 , "ol" , TokenType::BOOL);
            case 'r': return check_keyword(2 , 3 , "eak" , TokenType::BREAK);
            default: ;
            }
        }
        break;
    ...
    case 'c':
        if (scanner.current - scanner.start > 1)
        {
            switch (scanner.start[1])
            {
            case 'h': return check_keyword(2 , 2 , "ar" , TokenType::CHAR);
            case 'o':
                if (scanner.current - scanner.start > 2 && scanner.start[2] == 'n')
                {
                    if (scanner.current - scanner.start > 3)
                    {
                        ...
                        {
                        case 's': return check_keyword(4 , 1 , "t" , TokenType::CONST);
                        case 't': return check_keyword(4 , 4 , "inue" , TokenType::CONTINUE);
                        default: ;
                        }
                    }
                }
        ...
        break;
        //Check do
    case 'd': return check_keyword(1 , 1 , "o" , TokenType::DO);
        //Check else, enum
    case 'e':
        if (scanner.current - scanner.start > 1)
        {
    ...
    case 'f':
        if (scanner.current - scanner.start > 1)
        {
            switch (scanner.start[1])
            {
            case 'n': if (scanner.current - scanner.start == 2) return TokenType::FN;
                break;
            case 'a': return check_keyword(2 , 3 , "lse" , TokenType::FALSE);
            case 'o': return check_keyword(2 , 1 , "r" , TokenType::FOR);
            case '3': return check_keyword(2 , 1 , "2" , TokenType::F32);
            case '6': return check_keyword(2 , 1 , "4" , TokenType::F64);
            default: ;
            }
        }
        break;
    ...
    case 'i':
        if (scanner.current - scanner.start > 1)
        {
            switch (scanner.start[1])
            {
            case 'f': if (scanner.current - scanner.start == 2) return TokenType::IF;
                break;
            case 'n': if (scanner.current - scanner.start == 2) return TokenType::IN;
                break;
            case '8': if (scanner.current - scanner.start == 2) return TokenType::I8;
                break;
            case '1': return check_keyword(2 , 1 , "6" , TokenType::I16);
            case '3': return check_keyword(2 , 1 , "2" , TokenType::I32);
            case '6': return check_keyword(2 , 1 , "4" , TokenType::I64);
            default: ;
            }
        }
        break;
        //Check loop
    case 'l': return check_keyword(1 , 3 , "oop" , TokenType::LOOP);
        //Check match
    case 'm': return check_keyword(1 , 4 , "atch" , TokenType::MATCH);
        //Check NIL
    case 'N': return check_keyword(1 , 2 , "IL" , TokenType::NIL_LITERAL);
        //Check return
            ...
            case 't':
                    if (scanner.current - scanner.start > 2 && scanner.start[2] == 'r')
                    {
                        if (scanner.current - scanner.start > 3)
                        {
                            switch (scanner.start[3])
                            {
                            case 'i': return check_keyword(4 , 2 , "ng" , TokenType::STRING);
                            case 'u': return check_keyword(4 , 2 , "ct" , TokenType::STRUCT);
                            default: ;
                            }
                        }
                    }
                    if (scanner.current - scanner.start > 2 && scanner.start[2] == 'a')
                    {
                        if (scanner.current - scanner.start > 3)
                        {
                            switch (scanner.start[3])
                            {
                            case 't': return check_keyword(4 , 2 , "ic" , TokenType::STATIC);
                            default: ;
                            }
                        }
                    }
    ...
    case 'u':
        if (scanner.current - scanner.start > 1)
        {
            switch (scanner.start[1])
            {
            case '8': if (scanner.current - scanner.start == 2) return TokenType::U8;
                break;
            case '1': return check_keyword(2 , 1 , "6" , TokenType::U16);
            case '3': return check_keyword(2 , 1 , "2" , TokenType::U32);
            case '6': return check_keyword(2 , 1 , "4" , TokenType::U64);
            case 'n': return check_keyword(2 , 3 , "ion" , TokenType::UNION);
            default: ;
            }
        }
        break;
        //Check void
    case 'v': return check_keyword(1 , 3 , "oid" , TokenType::VOID);
        //Check while
...
Token scan_token()
{
    ...
    if (is_at_end()) return create_token(TokenType::END_OF_FILE);
    ...
    case '.': return create_token(match('.') ? TokenType::DOT_DOT : TokenType::DOT);
        // Arithmetic & Assignment Operators
    case '+': return create_token(match('=') ? TokenType::PLUS_EQUAL : TokenType::PLUS);
    case '*': return create_token(match('=') ? TokenType::STAR_EQUAL : TokenType::STAR);
    case '/': return create_token(match('=') ? TokenType::SLASH_EQUAL : TokenType::SLASH);
    case '%': return create_token(match('=') ? TokenType::PERCENT_EQUAL : TokenType::PERCENT);
        // Minus, Arrow, and Minus-Equal
    case '-':
        if (match('>')) return create_token(TokenType::ARROW);
        if (match('=')) return create_token(TokenType::MINUS_EQUAL);
        return create_token(TokenType::MINUS);
        // Comparison & Assignment
    case '=': return create_token(match('=') ? TokenType::EQUAL_EQUAL : TokenType::EQUAL);
    case '!': return create_token(match('=') ? TokenType::BANG_EQUAL : TokenType::BANG);
        //Less than, Less than equal, Left shift
    case '<':
        if (match('<')) return create_token(TokenType::LEFT_SHIFT);
        if (match('=')) return create_token(TokenType::LESS_EQUAL);
        return create_token(TokenType::LESS);
        //Greater than, Greater equal, Right shift
    case '>':
        if (match('>')) return create_token(TokenType::RIGHT_SHIFT);
        if (match('=')) return create_token(TokenType::GREATER_EQUAL);
        return create_token(TokenType::GREATER);
    case '&': return create_token(match('&') ? TokenType::AND : TokenType::BIT_AND);
    case '|': return create_token(match('|') ? TokenType::OR : TokenType::BIT_OR);
    case '^': return create_token(TokenType::BIT_XOR);
    case '~': return create_token(TokenType::BIT_NOT);
    ...
    default:
        if (is_digit(c)) return is_number_literal();
        if (is_alpha(c)) return is_identifier();
